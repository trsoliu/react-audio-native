# ADR 0003: Allow an explicit maintainer assessment for stable publication
...
HarmonyOS WebView and HarmonyOS NEXT ArkWeb before stable publication. The beta package and shared
core have since passed the complete code, documentation, package and Playwright browser matrix, and
clean registry consumers have built successfully with Vite, Nuxt, React and Next. The projects use
capability detection, native-control fallback and a protocol-neutral Bridge, but those automated
checks cannot fully reproduce host lifecycle or system audio routing on physical devices.
...
The 1.0.0 decision is recorded in `docs/device-smoke.md`. Every release still runs the full security,
lint, type, coverage, build, Silen, package and browser gates and publishes only through GitHub OIDC
Trusted Publishing with provenance. The workflow additionally proves the triggering push is an
allowlisted Changesets version transition with a removed package changeset, stable version
progression and no source files, then reconfirms the live default-branch head before publication.
