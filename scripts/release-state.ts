...
function consumedChangesets(state: ChangesetsPrereleaseState): string[] | null {
  if (
    state.mode !== 'pre' ||
    state.tag !== 'beta' ||
...
export async function inspectReleaseState(
  ...
  const consumedChangesets = new Set(prereleaseState.changesets as string[])
      ...
      : new Set(
          await Promise.all(
            manifestPaths.map(async (manifestPath): Promise<string> => {
              const manifest = await readCurrentJson<PackageVersionManifest>(
                repositoryRoot,
                manifestPath,
              )
              if (
